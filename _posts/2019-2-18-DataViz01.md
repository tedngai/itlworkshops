...
![test image size](../../../assets/images/pic_jupyter_firstline.jpg){:height="50%" width="50%"}

<p>Python is a very powerful programming language due to the fact that it has a large community of developers writing libraries to handle lower level programming, allowing you to concentrate on the big ideas.

There're many libraries and packages out there that will handle different types of programming issues. For us, the first step is to install the packages as we did earlier using pip install. And when it's time to program, we need to tell python that these packages are already installed and we need to call them into the current system.</p>
