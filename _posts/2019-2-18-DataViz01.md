...
try to see how many **Titles** there are, and if we can visualize salary levels based on the Title. To do this we need to figure out how many different Titles there are in that column. You can do this in many different ways but a powerful and pythonic way is to write a function. This following is an example to list all the duplicates and in which record the duplicate appears. For now, we're not going to go into the details so we'll just assume this is a button in a computer software you can press and it'll get you what you need.
