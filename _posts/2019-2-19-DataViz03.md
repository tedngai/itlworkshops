...
<pre><code>test_nullValue = []
for row in (df_moma.DateAcquired):
    q = re.search(r'(\d{4})-(\d{1,2})-(\d{1,2})$', row)
    if not q:
        print(row)
        test_nullValue.append(row)</code></pre>
...
list_duplicates(test_nullValue)</code></pre>

Here, a **list_duplicate** function is create to look for unique items. And low and behold, we get this as a result. `['Unknown', '1977-08', '1975-01', '1994', '1961']`. So other than Unknown, we also have 2 other date format we have to account for.
